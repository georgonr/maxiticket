...
import { ArrowLeft, Trash2, Plus, FileDown, Lock } from 'lucide-react';
    ...
    <div className="min-h-screen bg-cream dark:bg-gray-950">
      <main className="mx-auto max-w-3xl space-y-6 p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
              {t('detailTitle')} {inv.invoiceNumber ?? t('noNumber')}
            </h1>
            <span className={`mt-1 inline-block rounded-full px-2 py-0.5 text-xs font-medium ${isDraft ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>
              {t(`status.${inv.status}`)}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={downloadPdf}><FileDown size={15} className="mr-1.5" /> {t('downloadPdf')}</Button>
        ...
        <SectionCard title={t('items')}>
          ...
          {/* Pridať CUSTOM (len DRAFT) */}
          {isDraft && (
            <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-[1fr_70px_90px_70px_auto] sm:items-end">
              ...
              <Button variant="outline" onClick={addItem} disabled={busy}><Plus size={15} /></Button>
            </div>
          )}
        ...
        {/* Hlavička (DRAFT edit) */}
        <SectionCard title={t('detailTitle')}>
