...
import { billingApi, InvoiceListRow } from '@/lib/api/billing';
...
export default function AdminInvoicesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(await billingApi.listInvoices(token));
    } catch {
      setError(tb('errLoad'));
    } finally {
      setLoading(false);
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                  {rows.map((r) => (
                    <tr key={r.id} onClick={() => router.push(`/admin/billing/invoices/${r.id}`)} className="cursor-pointer hover:bg-cream/60 dark:hover:bg-gray-800">
                      ...
                      <td className="px-3">
                        <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${r.status === 'FINALIZED' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                          {t(`status.${r.status}`)}
                        </span>
                      </td>
