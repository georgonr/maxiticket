...
import { Calendar, MapPin, Lock, Minus, Plus, Ticket } from 'lucide-react';
import { publicApi, QrTicketInfo } from '@/lib/api';
import { localizeApiError } from '@/lib/api-error';
import { Button } from '@/components/ui/button';

/**
 * Krok 50 (V5): údaje o type lístka + cena prídu zo SERVERA (SSR, initialInfo) –
 * v úvodnom HTML, odolné voči ad-blockeru. loadStatus rozlišuje 404 od výpadku.
 */
export function QrBuyClient({
  ticketTypeId,
  initialInfo,
  loadStatus,
  qtyParam,
  ...
  const [qty, setQty] = useState(
    Number.isFinite(qtyParam) && qtyParam >= 1 && initialInfo && initialInfo.maxQuantity > 0
      ? Math.min(Math.floor(qtyParam), initialInfo.maxQuantity)
      : 1,
  );
  ...
  // Rozlíšenie (V6): „nenašlo sa" (404) vs „nepodarilo sa načítať" (výpadok) – NIE prázdno.
  if (!info) {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        <Ticket className="mx-auto mb-3 text-gray-300" size={40} />
        <p className="text-gray-600 dark:text-gray-300">
          {loadStatus === 'error' ? t('loadError') : t('notFound')}
        </p>
      </div>
    );
  }
