...
/**
 * Krok 50 (V5): počiatočný zoznam podujatí + mestá prídu zo SERVERA (SSR, initial*),
 * takže úvodné vykreslenie NIE je prázdne ani pri blokovanom api.*. Filtre/search/
 * kalendár ostávajú klientske (klientsky re-fetch len pri zmene filtra).
 */
export function EventsClient({
  ...
  initialShows: PublicShow[];
  initialCities: string[];
  initialLoadError: boolean;
  initial: { q: string; category: string; city: string; date: string };
}) {
  const t = useTranslations('events');
  const [shows, setShows]           = useState<PublicShow[]>(initialShows);
  const [loading, setLoading]       = useState(false);
  const [fetchError, setFetchError] = useState(initialLoadError);
  const [cities]                    = useState<string[]>(initialCities);
  // Filtre sa inicializujú z URL query (SSR ich prečítal a odovzdal cez `initial`).
  const [filterCat, setFilterCat]   = useState(initial.category);
  const [filterDate, setFilterDate] = useState(initial.date);
  const [filterCity, setFilterCity] = useState(initial.city);
  const [filterQ, setFilterQ]       = useState(initial.q);
  const [qInput, setQInput]         = useState(initial.q);
  const [view, setView]             = useState<'grid' | 'calendar'>('grid');
  const [reloadKey, setReloadKey]   = useState(0);  // retry po zlyhaní fetchu

  // SSR už načítal zoznam pre počiatočné filtre → prvý beh fetch efektu preskoč,
  // aby sa (a) zbytočne neopakoval fetch a (b) prípadné zlyhanie NEprepísalo SSR dáta.
  ...
  useEffect(() => {
    if (seededRef.current) { seededRef.current = false; return; } // SSR seed – neťahaj znova
    setLoading(true);
    setFetchError(false);
    publicApi
      .listShows({
      ...
      .then((s) => { setShows(s); setFetchError(false); })
      // Rozlíš chybu od prázdna (V6): pri zlyhaní NEnastavuj [] („žiadne podujatia"),
      // ale error flag → zobrazí sa „nepodarilo sa načítať".
      .catch(() => setFetchError(true))
      .finally(() => setLoading(false));
  }, [filterQ, filterCat, filterDate, filterCity, view, reloadKey]);
      ...
      <div className="px-4 sm:px-6 py-8">
        <div className="mx-auto max-w-7xl">
          {loading ? (
            ...
            // Chyba načítania (V6) – NIE „žiadne podujatia", ktoré vyzerá ako že zmizli.
            <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 py-24 text-center">
              <Loader2 size={40} className="mb-4 text-slate-300" />
              <p className="text-lg font-semibold text-slate-500">{t('loadErrorTitle')}</p>
              <p className="mt-1 text-sm text-slate-400">{t('loadErrorHint')}</p>
              <button
