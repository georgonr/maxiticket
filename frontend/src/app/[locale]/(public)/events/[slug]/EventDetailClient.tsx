...
import {
  Calendar, MapPin, Clock, Plus, Minus, ShoppingCart,
  ChevronRight, ChevronLeft, Tag, AlertCircle, AlertTriangle, CheckCircle2,
  Share2, Copy, Check, MessageCircle,
} from 'lucide-react';

/**
 * Krok 50 (V5): dáta podujatia prídu zo SERVERA (SSR, initialShow) – v úvodnom HTML,
 * crawlable a odolné voči ad-blockeru. Interaktivita (výber lístka/sedadla, košík)
 * ostáva klientska. loadStatus rozlišuje „nenašlo sa" (404) od „nepodarilo sa načítať".
 */
export function EventDetailClient({
  ...
  slug: string;
  initialShow: PublicShowDetail | null;
  loadStatus: 'ok' | 'notfound' | 'error';
}) {
  ...
  // Zo servera – žiadny klientsky initial fetch (odolné voči blokovaniu api.*).
  const show = initialShow;
  const [selectedTermin, setSelectedTermin] = useState<PublicTerminDetail | null>(
    initialShow?.termins[0] ?? null,
  );
  ...
  // ── Not-found (404) vs chyba načítania (výpadok) ────────────────────────────
  // Rozlíšenie (V6): „nenašlo sa" vs „nepodarilo sa načítať" – NIE prázdna stránka.
  if (!show) {
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
        ...
        <h1 className="text-xl font-semibold text-slate-700">
          {isError ? t('loadErrorTitle') : t('notFoundTitle')}
        </h1>
        <p className="mt-1 text-sm text-slate-400">
          {isError ? t('loadErrorDesc') : t('notFoundDesc')}
        </p>
        <Link
          href="/events"
